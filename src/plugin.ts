import type { Hooks, PluginInput, AuthHook, ProviderContext } from "@opencode-ai/plugin"
...
import { IFLOW_CONSTANTS, applyThinkingConfig, registerIFlowModels } from "./constants"
  ...
  async (input: PluginInput): Promise<Hooks> => {
    ...
    return {
              ...
              while (true) {
                ...
                if (
                  ...
                  try {
                    ...
                    logger.error(
                      `Token refresh failed for account ${acc.id}`,
                      error,
                    )
                    am.markUnhealthy(
                      acc,
                ...
                try {
                  ...
                  if (response.status === 429) {
                    ...
                    logger.warn(
                      `Rate limited on account ${acc.email}, retry after ${retryAfter}s`,
                    )
                  ...
                  if (response.status === 401 || response.status === 403) {
                    logger.warn(
                      `Authentication failed for ${acc.email}: ${response.status}`,
                    )
                    am.markUnhealthy(
                      acc,
                  ...
                  if (response.status >= 500) {
                    if (retry < 3) {
                      retry++
                      logger.warn(
                        `Server error ${response.status}, retry ${retry}/3`,
                      )
                      await sleep(1000 * Math.pow(2, retry))
                      continue
                    }
                    logger.error(
                      `Server error ${response.status} after ${retry} retries`,
                    )
                  ...
                  throw new Error(
                    `iFlow Error: ${response.status} - ${errorText}`,
                  )
                } catch (error: any) {
                  if (isNetworkError(error) && retry < 3) {
                    retry++
                    logger.warn(
                      `Network error, retry ${retry}/3: ${error.message}`,
                    )
                    await sleep(1000 * Math.pow(2, retry))
                    continue
                  }
                  ...
                  logger.error(
                    `Request failed after ${retry} retries: ${error.message}`,
                    error,
                  )
                  throw error
                }
                  ...
                  while (true) {
                    ...
                    > => {
                      try {
                        ...
                        const { url, redirectUri, waitForAuth } =
                        ...
                        logger.error(
                          `OAuth authorization failed: ${e.message}`,
                          e,
                        )
            ...
            authorize: async (inputs?: any) => {
              ...
              while (true) {
                ...
                try {
                  ...
                  showToast(
                    `Account ${accounts.length} added (${email})`,
                    "success",
                  )
                  ...
                  try {
                    ...
                    logger.warn(
                      `Failed to load account count: ${e.message}`,
                    )
                  }
      ...
      "chat.headers": async (
        ...
        if (input.model.providerID !== IFLOW_PROVIDER_ID) return
